...
|---|---|---|---|
| BUG-01 | 🟡 Medium | Negative integer inputs (likes, comments, shares, saves) are not blocked by the UI — `number_input` has `min_value=0` so the UI prevents it, but no server-side guard exists in `diagnostic_engine.py`. If passed programmatically, negative likes produce a negative engagement rate (e.g., `-0.033`). | Live test: `likes=-50` → `engagement rate: -0.033`, `per100 likes: -5.0` |
| BUG-02 | 🟢 Low | `follower_count=0` routes to the `under_1k` tier and returns a valid label. Functionally safe but semantically wrong — a creator with 0 followers should not receive a scored engagement label. | Live test: `follower_count=0` → `label: Good` |
| BUG-03 | 🟡 Medium | OpenAI API calls in `ai_report.py` have no `timeout` parameter. On a slow network or OpenAI degradation, the call hangs indefinitely until the server or user kills it. `except Exception` catches the error eventually but with no time bound. | Code audit: `client.chat.completions.create(model=..., max_tokens=750)` — no `timeout=` kwarg on any of the 3 call sites |
...
| ID | Severity | Finding | Evidence |
|---|---|---|---|
| UX-01 | 🟢 Low | All `number_input` fields have `min_value=0`, preventing negative UI input. However `follower_count` defaults to `0` — a first-time user who forgets to set it gets scored against `under_1k` benchmarks silently. No warning shown when follower count is 0. | Code audit line 1736: `min_value=0, value=0` |
| UX-02 | 🟢 Low | The `views=0` edge case returns all-zero scores with no user-facing message explaining why results are empty. | Live test TC-01: `engagement rate: 0, retention: 0` — no error raised |
...
| ID | Severity | Finding | Evidence |
|---|---|---|---|
| PERF-01 | 🟡 Medium | No timeout on OpenAI calls — same as BUG-03. In production, a hanging AI call blocks the entire Streamlit session for that user. | `ai_report.py` lines 87, 154, 255 — zero `timeout=` kwargs |
...
| TC-05 | Negative likes input | Security/Edge | `views=1000, likes=-50` | Should block or clamp to 0 | No crash but rate returned as `-0.033` — **BUG-01 confirmed** | ✗ FAIL |
...
| TC-15 | OpenAI timeout present | Performance | Code audit `ai_report.py` | `timeout=` param on API calls | **Not found on any of 3 call sites — BUG-03 confirmed** | ✗ FAIL |

**Result: 12 PASS · 2 FAIL · 1 WARN**
...
| 6 | Negative values safe at engine level | NO | BUG-01 — no server-side clamp |
| 7 | OpenAI calls have timeout protection | NO | BUG-03 — no `timeout=` kwarg |
...
**Readiness Score: 18/20 YES · 2 NO (both fixable in < 30 minutes)**
...
| ID | Issue | Fix | Effort |
|---|---|---|---|
| BUG-01 | Negative likes produce negative engagement rate at engine level | Add `max(0, value)` clamp in `diagnostic_engine.py` before calculations | ~5 min |
| BUG-03 / PERF-01 | OpenAI calls have no timeout — can hang indefinitely | Add `timeout=30` to all 3 `client.chat.completions.create()` calls in `ai_report.py` | ~5 min |
...
- Two medium-severity issues have been identified and are documented above
- The system is stable, functional, and ready for final academic evaluation subject to the two outstanding fixes noted above
